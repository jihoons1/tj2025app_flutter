...
void main() { // main 스레드 갖는 함수
  ...
  bool isStudent = false; // boolean java xx android에선 bool
  String name = "유재석";  // 문자열 String
  print(" 이름 : ${ name } , 키 : ${ height }, 학생여부 : ${ isStudent } ");
  // [2] 주요 변수/상수 키워드
  var city = "서울";    // JS처럼 자동 타입 추론 - 타입변경불가능
  dynamic count = 123;  // 자동 타입 + 타입변경
  count = "123명";
  const PI = 3.14159;   // JS처럼 count 상수 (런타입 시점)
  final country = "대한민국"; // JAVA 처럼 FINAL 상수 (컴파일 시점)

  // [3] List , Map ,  Set ( 컬렉션 )
  List<String> friits = ["사과" , "바나나" , "포도" ];
  friits.add("딸기");
  print("과일 목록 : ${ friits }");
  ...
  // int : 반환 타입 , add : 함수명 , int a / int b : 매개변수
  int result = add( 3 , 5 ); print( result); // 8
  // 익명함수/화살표함수 dynamic 함수명 = (){}
  ...
  // new 위젯명( 속성값 , 속성값 ) , new 생략 가능 , 위젯명( 위젯명( 위젯명( ) )  )
