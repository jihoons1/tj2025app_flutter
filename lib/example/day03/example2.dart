...
class GoodsWidgetState extends State<GoodsWidget>{
  ...
  void goodsSave() async{
    ...
    print( obj );
    try{
      final dio = Dio(); // [1] Dio 객체 생성한다. // [2] 주의할점 : web[HTTP있음] 모바일[HTTP없음]
      // (web) 픽드시 : localhost:8080 가능 . (mobile) : 192.168.40.185  가능
      // 스프링 컨트롤러 에서 @CrossOrigin(origins = "*") 또는 corsConfig
      final response = await dio.post("http://192.168.40.24:8080/api/goods" , data: obj);
  ...
  // [4-3] goodAll 함수 정의 , useEffect( ()=> {} [] ) 처럼 위젯이 최초로 열렸을때 1번 실행
  void goodsAll() async{
    try{
      final dio = Dio();
      final response = await dio.get("http://192.168.40.24:8080/api/goods/list");
      final data = response.data;
      print( data ); // 통신 결과
  ...
  @override
  void initState() { goodsAll(); } // 현재 위젯이 최초로 실행될떄 , 1번 실행
  // [4-5] dio 로 가져온 데이터를 저장하는 상태 변수
  dynamic goodsList = [];
  // dynamic : Object 같은 친구
      ...
      child: Column(
        children: [
          ...
          Expanded(child: ListView.builder(
              itemCount: goodsList.length , // 리스트의 총 개수
